<template>
  <el-dialog
    ...
    <div class="uploadItem" style="margin-top: 15px">
      <div style="display: flex;align-items: center;justify-content: center">训练数据集</div>
    ...
    <div class="uploadItem" style="margin-top: 15px">
      ...
      <el-input placeholder="本地训练数据集路径" size="medium" v-model="path" style="width: 300px; margin-left: 20px"/>
    </div>
...
export default {
  ...
  methods: {
    getDatasetsList() {
      getDatasetsListR(1, 1).then(data => {
        if (data.state === 100) {
    ...
    ok() {
      ...
      if (data.datasetsId == '0') {
        if (!data.path) {
          this.$message.error("请输入本地训练数据集路径")
          return;
        }
      }
      this.$emit("ok",data)
